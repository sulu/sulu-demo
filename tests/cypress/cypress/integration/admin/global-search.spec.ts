...
describe("Use global search", () => {
    ...
    it.only("search for testify will have result 0 in api response", () => {
        globalSearchPage.searchInputField.type("testify{enter}");
        cy.wait("@search").then((interception) => {
            expect(interception.response.body.total).equals(0);
        });
    });
});
